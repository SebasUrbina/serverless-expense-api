...
export class UserSetup extends OpenAPIRoute {
  ...
  async handle(c: AppContext) {
    ...
    try {
      ...
      const accountsCount: any = await db
        .prepare('SELECT COUNT(*) as count FROM accounts WHERE user_id = ?')
        .bind(userId)
        .first();
      const categoriesCount: any = await db
      ...
      const categoryStatements = defaultCategories.map((cat) =>
        db
          ...
          .bind(userId, cat.name, cat.type, cat.icon),
      );
      ...
      return {
        setup: true,
        message: 'Default configuration applied successfully',
      };
    } catch (error: any) {
