...
export class GroupSettle extends OpenAPIRoute {
      ...
      body: {
        content: {
          'application/json': {
            schema: z.object({
              month: Str({
                description: 'YYYY-MM to settle',
                example: '2026-03',
              }),
              account_id: Num({
                description: 'Account to record settlement against',
              }),
              settlements: z
                .array(
                  z.object({
                ...
                .optional()
                .describe('Specific pairs to settle. If omitted, settle all.'),
            }),
          },
        },
      },
  ...
  async handle(c: AppContext) {
    ...
    const group = (await c.env.DB.prepare(
      `SELECT name FROM shared_groups WHERE id = ?`,
    )
      .bind(groupId)
      .first()) as any;
    if (!group) {
    ...
    for (const member of members) {
      const paidResult = (await c.env.DB.prepare(
        ...
        .bind(member.user_id, groupId, startDate, endDate)
        .first()) as any;
    ...
    if (settlementAmount === 0) {
      return c.json(
        { success: false, error: 'Nothing to settle — balances are even' },
        400,
      );
    }
    ...
    const pairsToSettle =
      requestedSettlements && requestedSettlements.length > 0
        ? settlements.filter((s) =>
            requestedSettlements.some(
              (rs) =>
                rs.debtor === s.debtor &&
                rs.creditor === s.creditor &&
                rs.amount === s.amount,
            ),
          )
        : settlements;
    ...
    if (pairsToSettle.length === 0) {
      return c.json(
        { success: false, error: 'No matching settlement pairs found' },
        400,
      );
    }
