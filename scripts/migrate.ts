...
const CSV_FILE = 'data/Gasto - Movimientos - 20260419.csv';

const API_URL = ''; // process.env.API_URL || 'http://127.0.0.1:8787';
const TOKEN = process.env.TOKEN;
const API_KEY = '0'; // process.env.API_KEY;

if (!TOKEN && !API_KEY) {
  console.error(
    'Por favor, provee un TOKEN o un API_KEY (ej. API_KEY=tu_key npx tsx scripts/migrate.ts)',
  );
  process.exit(1);
}
...
async function migrate() {
  ...
  const resolveCategory = async (csvCat: string, csvTipo: string) => {
    ...
    if (
      csvTipo.toLowerCase().includes('ingreso') ||
      csvCat === 'Pension' ||
      csvCat === 'Ingreso'
    ) {
      type = 'income';
    ...
    let existing = categories.find(
      (c: any) =>
        c.name.toLowerCase() === mappedName.toLowerCase() && c.type === type,
    );
    if (!existing) {
      console.log(`Creando categoría ${mappedName} (${type})...`);
      const rs = await apiCall('/api/categories', 'POST', {
        name: mappedName,
        type,
        icon: '📦',
      });
  ...
  const resolveAccount = async (csvCuenta: string) => {
    ...
    if (!existing) {
      ...
      let type = 'checking';
      if (
        accountName.toLowerCase().includes('tarjeta') ||
        accountName.toLowerCase().includes('visa') ||
        accountName.toLowerCase().includes('credito')
      )
        type = 'credit';
      else if (accountName.toLowerCase() === 'efectivo') type = 'cash';

      const rs = await apiCall('/api/accounts', 'POST', {
        name: accountName,
        type,
        balance: 0,
      });
  ...
  for (let i = 0; i < records.length; i++) {
    ...
    if (amount === null || amount <= 0) {
      skipped++;
      errorRecords.push({ ...row, _error: 'Monto nulo o <= 0' });
    ...
    const isIncome =
      row['Gasto/Ingreso']?.toLowerCase() === 'ingreso' ||
      csvCategory === 'Pension' ||
      csvCategory === 'Ingreso';
    ...
    try {
      ...
      if (rs.status === 200) {
        ...
        if (imported % 50 === 0)
          console.log(`... importadas ${imported} transacciones`);
      } else {
        console.error(`\nError importando fila ${i + 2} (${title}):`, rs.data);
        errorRecords.push({
          ...row,
          _error: `API Status ${rs.status}: ${JSON.stringify(rs.data)}`,
        });
      }
      ...
      errorRecords.push({ ...row, _error: `Excepción: ${e.message}` });
    }
  }
  ...
  if (errorRecords.length > 0) {
    const errorCsvPath = path.resolve(
      process.cwd(),
      'data/migration_errors.csv',
    );
    const headers = Object.keys(errorRecords[0]);
    const csvContent = [
      headers.join(','),
      ...errorRecords.map((r) =>
        headers
    ...
    fs.writeFileSync(errorCsvPath, csvContent);
    console.log(
      `\n⚠️ Se generó '${errorCsvPath}' con ${errorRecords.length} registros que fallaron.`,
    );
  }
}
